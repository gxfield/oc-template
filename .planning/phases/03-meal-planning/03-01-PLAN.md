...
- "tonight" / "today": Get current day name from `node calendar.js now`, find matching line in this-week.md
- "tomorrow": Get current day name from `node calendar.js now`, compute next day, find matching line
...
Response: "Updated! Thursday dinner is now chicken parmesan 🍽️"

Example 2 - Query tonight:
Input: "what's for dinner tonight"
Action: Run `node calendar.js now` to get day name (e.g., "Sunday"), read this-week.md, find "- Sunday: Leftover buffet"
Response: "Tonight's dinner: Leftover buffet 🍽️"

Example 3 - No plan for today:
Input: "what's for dinner"
Action: Get day name, find "- Wednesday: No plan"
Response: "No dinner planned for tonight. Want me to set something? 🍽️"
...
Response: "Updated 3 meals! Monday: Tacos, Tuesday: Pasta, Wednesday: Salmon 🍽️"
...
| DO | DO NOT | WHY |
|---|---|---|
| Get today's day name from `node calendar.js now` | Use `date` command or guess the day | System clock is UTC; wrong day after 4 PM Pacific |
...
- "set dinner to pizza" with no day specified -> assume tonight (get from `node calendar.js now`)
...
- `node calendar.js now` referenced for day name lookups (not `date` command)
  </verify>
